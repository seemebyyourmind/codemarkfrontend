...
const GroupDetail = () => {
    ...
    return (
        ...
        <div className="flex flex-wrap gap-5 xl:gap-20">
            ...
            <Link
              to="./statistics"
              className={`inline-flex items-center justify-center rounded-full py-4 px-10 text-center font-medium lg:px-8 xl:px-10 ${location.pathname.endsWith('/statistics') ? 'bg-white text-black dark:bg-slate-200' : 'bg-black text-white hover:bg-opacity-90 dark:bg-slate-500'}`}
            >
            Thống kê
            </Link>
        </div>
